for(k=1; k<9; k++){
  ...
  story.innerHTML = `
    <li><img src="img/story_img${k}.PNG" alt="x"></li>
...
// 그냥 div가 아닌 feed_box에 css를 적용시켰기 때문에 클래스 이름을 생성해줘야 함.
feedBox.innerHTML = `
<div class="feed_id">
        <div class="id">
         <ul>
          <a href="#">
           <li><img src="./img/story_img${i}.PNG" alt="x"></li>
           <li>
            <p>Instar_ID${i}</span></p>
            <p>·${i}</span>시간</p></li>
          </a>
         </ul>
        </div>
       ...
       <!-- feed_id end -->
        ...
        <!-- feed_icon end -->
        <div class="feed_txt">
        <p><span>Instar_ID${i}</span></p>
        <p class="txt">여기는 피드내용을 작성하는 영역입니다. 원하시는 문구를 자유롭게 작성해 보세요!</p><span class="txtAdd">더보기</span>
        </div>
        <!-- feed_txt -->
...
feedBoxHandler(feedBox)
}



//like를 눌렀을 때, hate 나타나고 like 사라지기
//자바스크립트로 만들기
//like,hate 불러오기 >> document.getElementById()
//like를 클릭하면 hate 나타나고 (display:block), like 사라지기(display:none)
//클릭이벤트 addEventListner('click', 함수)
function feedBoxHandler(feedBox){
